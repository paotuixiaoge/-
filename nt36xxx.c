...
static struct tp_node_rw_callback_st nvt_node_callback[TP_NODE_NUM];
...
extern int send_prox_rawdata(u8 data);
#endif
...
static int32_t get_lockdown_info_by_lcd(void)
{
    ...
    if (lcm_name) {
        ...
        temp1 = strstr(videolfb_tag->lcmname,"tp_lockdown_info=");
        if(temp1 != NULL) {
            temp2 = strstr(temp1,"=");
    ...
    NVT_LOG("read tp_lockdown_info = %s\n", lockdown_info);
    return 0;
}
#endif
...
static void nvt_battery_status_work(struct work_struct *work)
{
    int ret;
...
int32_t nvt_panel_index = 0;
int32_t nvt_get_panel_name(void)
{
	struct device_node *cmdline_node;
	const char *cmd_line, *lcd_name_p;
	char lcd_name[50];
	int rc;
	int32_t status = TM_PANEL;
	...
	if (!rc) {
		...
		if (lcd_name_p) {
			...
			if (strstr(lcd_name, "dsi_panel_somaliaa_36_02_0a_hdp_vdo") != NULL) {
				NVT_LOG("we will use TM panel\n");
				status = TM_PANEL;
			} else if (strstr(lcd_name, "dsi_panel_somaliaa_42_02_0b_hdp_vdo") != NULL) {
				NVT_LOG("we will use HX panel\n");
				status = HX_PANEL;
			} else {
				NVT_ERR("can not match panel, we will use default(TIANMA)!!!\n");
				status = TM_PANEL;
			}
		} else {
			NVT_ERR("can't find lcd_name\n");
		}
	} else {
		NVT_ERR("can't parse bootargs property\n");
	}
...
#if LCT_TP_INFO
	if (!nvt_panel_index) {
		sprintf(tp_info_buf, "[Vendor]tianma,[FW]0x%02x,[IC]nt36528a\n", ts->fw_ver);
		NVT_LOG("[Vendor]tianma,[FW]0x%02x,[IC]nt36528a\n", ts->fw_ver);
	} else {
		sprintf(tp_info_buf, "[Vendor]huaxin,[FW]0x%02x,[IC]nt36528a\n", ts->fw_ver);
		NVT_LOG("[Vendor]huaxin,[FW]0x%02x,[IC]nt36528a\n", ts->fw_ver);
        }
...
static void nvt_esd_check_func(struct work_struct *work)
{
	...
	if ((timer > NVT_TOUCH_ESD_CHECK_PERIOD) && esd_check) {
		...
		if (!nvt_panel_index)
			nvt_update_firmware(TM_BOOT_UPDATE_FIRMWARE_NAME);
		else
			nvt_update_firmware(HX_BOOT_UPDATE_FIRMWARE_NAME);
		mutex_unlock(&ts->lock);
		/* update interrupt timer */
		irq_timer = jiffies;
...
int32_t nvt_check_proximity(uint8_t input_id, uint8_t *data)
{
	...
	if ((input_id == DATA_PROTOCOL) && (func_type == FUNCPAGE_PROXIMITY)) {
		...
		if (proximity_state == PROXIMITY_ON) {
			...
			proximity_level = 3;
#endif
			send_prox_rawdata(proximity_level);
			NVT_LOG("get proximity on near event, proximity_level=%d.\n", proximity_level);
		} else if (proximity_state == PROXIMITY_OFF) {
			proximity_level = 0;
			send_prox_rawdata(proximity_level);
			NVT_LOG("get proximity on far event, proximity_level=%d.\n", proximity_level);
		} else {
			// should never go here
...
#ifdef UNISOC_TOUCHSCREEN
static ssize_t nvt_ts_dbg_log_show(struct device *dev, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%s\n", nvt_debug_flag ? "true" : "false");
}

static ssize_t nvt_ts_dbg_log_store(struct device *dev, int input)
{
	nvt_debug_flag = input;
	return 0;
}

static ssize_t nvt_ts_input_name_show(struct device *dev, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%s\n", NVT_SPI_NAME);
}

/*
static ssize_t nvt_ts_suspend_show(struct device *dev, char *buf)
{

	return snprintf(buf, PAGE_SIZE, "%s\n", bTouchIsAwake ? "false" : "true");
}

static ssize_t nvt_ts_suspend_store(struct device *dev, int input)
{
	if (input == 1)
		nvt_ts_suspend(dev);
	else if (input == 0)
...
static ssize_t nvt_ts_firmware_version_show(struct device *dev, char *buf)
{
	return sprintf(buf, "0x%02X\n", ts->fw_ver);
}

static int nvt_sprd_notifier_callback(struct notifier_block *nb,
		unsigned long action, void *data)
{
	struct nvt_ts_data *ts = container_of(nb, struct nvt_ts_data, panel_notif);
	int *input = data;
	uint32_t i = 0;
	...
	if (*input == 1) {
		if (!proximity_event) {
			...
			NVT_LOG("proximity_event = %d, tp is proximity suspend\n",proximity_event);
			bTouchIsAwake = 0;
#if NVT_TOUCH_ESD_PROTECT
...
#endif
	nvt_panel_index = nvt_get_panel_name();
...
#if LCT_TP_USB_PLUGIN
	//ts->event_wq = alloc_workqueue("nvt-event-queue", WQ_UNBOUND | WQ_HIGHPRI, 1);
	INIT_WORK(&ts->online_work, nvt_battery_status_work);
...
#ifdef UNISOC_TOUCHSCREEN
	nvt_node_callback[TP_NODE_DBG_LOG].store = nvt_ts_dbg_log_store;
	nvt_node_callback[TP_NODE_DBG_LOG].show = nvt_ts_dbg_log_show;
	//nvt_node_callback[TP_NODE_TS_SUSPEND].store = nvt_ts_suspend_store;
	//nvt_node_callback[TP_NODE_TS_SUSPEND].show = nvt_ts_suspend_show;
	nvt_node_callback[TP_NODE_INPUT_NAME].show = nvt_ts_input_name_show;
	nvt_node_callback[TP_NODE_FIRMWARE_VERSION].show = nvt_ts_firmware_version_show;
	ret = sprd_tp_creat_node(&client->dev.kobj, nvt_node_callback);
	if (ret < 0) {
		NVT_ERR("Failed to crate node.\n");
		goto err_sysfs;
	}
	ts->panel_notif.notifier_call = nvt_sprd_notifier_callback;
	ret = blocking_notifier_chain_register(&panel_power_notifier_list, &ts->panel_notif);
	if (ret) {
		NVT_ERR("failed to register panel_notifier_list: %d", ret);
		goto err_register_notif_chain;
	}
#endif
#if LCT_TP_LOCKDOWN
...
#ifdef UNISOC_TOUCHSCREEN
	blocking_notifier_chain_unregister(&panel_power_notifier_list, &ts->panel_notif);
err_register_notif_chain:
	sprd_tp_remove_node(&client->dev.kobj);
err_sysfs:
#endif
...
#endif
	if (!nvt_panel_index)
		ret = nvt_update_firmware(TM_BOOT_UPDATE_FIRMWARE_NAME);
	else
		ret = nvt_update_firmware(HX_BOOT_UPDATE_FIRMWARE_NAME);
	if (ret) {
...
static int get_bootargs(char *current_mode, char *boot_param)
{
	...
	if (!np) {
		NVT_ERR("Can't get the /chosen\n");
		return -EIO;
	}
	ret = of_property_read_string(np, "bootargs", &cmd_line);
	if (ret < 0) {
		NVT_ERR("Can't get the bootargs\n");
		return ret;
	}
...
static int32_t __init nvt_driver_init(void)
{
	...
	if (ret) {
		NVT_ERR("%s:fail to get bootargs, return\n",__func__);
		return ret;
	}

	NVT_LOG("nvt_lcd_name = %s\n", lcd_name);

	if (strstr(lcd_name, "dsi_panel_somaliaa_36_02_0a_hdp_vdo"))
	{
		NVT_LOG("nvt:tianma+NT36528A,touch_lcm_name_success=%s\n",lcd_name);
	} else if (strstr(lcd_name, "dsi_panel_somaliaa_42_02_0b_hdp_vdo"))
	{
	...
	NVT_LOG("NVT_start\n");
	/*
	if (!(strstr(lcd_name, "dsi_panel_somaliaa_36_02_0a_hdp_vdo") || strstr(lcd_name, "dsi_panel_somaliaa_42_02_0b_hdp_vdo"))) {
		NVT_ERR("%s: match nt36528a fail, lcd name is %s\n",__func__,lcd_name);
		return ret;
	}
	*/
	//---add spi driver---
